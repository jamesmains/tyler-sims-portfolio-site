...
export const projects = sqliteTable("projects", {
    ...
    description: text("description").notNull(),             // Short description of the content                     (Listing)
    showcase: text("showcase").notNull(),                   // Main banner image for the content                    (Listing/Details)
    bodyContent: text("body_content").notNull(),            // Rich text content that allows markup                 (Details)
    gallery: text("gallery", {mode: 'json'}).notNull()      // JSON blob containing a list of filepaths for the     ...  
